...
class MainClass
{
    ...
    {
        $this->db = new mysqli('localhost', 'root', '', 'network_security1db');
        if (!$this->db) {
    ...
    public function login()
    {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($pass_is_right && (is_null($data['otp']) || (!is_null($data['otp']) && !is_null($data['otp_expiration']) && strtotime($data['otp_expiration']) < time()))) {
                ...
                $resp['status'] = 'failed';
                $_SESSION['flashdata']['type'] = 'danger';
                $_SESSION['flashdata']['msg'] = ' Cross-check your Logins!';
            }
        } else {
            $resp['status'] = 'failed';
            $_SESSION['flashdata']['type'] = 'danger';
            ...
            try {
                ...
                <html>
                    <body>
                        ...
                        <p>Here is your OTP (One-Time-Password) to verify your Identity.</p>
                        <h3><b>" . $pin . "</b></h3>
                    </body>
                </html>
                ";
